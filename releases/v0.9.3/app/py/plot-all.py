...
def main( ):
  ...
  while line:
    ...
    if len( fields ) > 0:
      ...
      cnt += 1
      if n_active_threads >= NUM_THREADS:
      ...
      try:
        ...
        if c_pid == 0:
          # the child
          try:
            #print(f"creating child {os.getpid()}")
          ...
          # the parent, take note of my child
...
def wait_children(children=[]):
  """
  Wait for all the child processes in the list to terminate
  
  :param      children:  List of pid's for children
  :type       children:  Array
  """
  for pid in children:
    try:
